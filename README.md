...
Project ini menggunakan beberapa _library_ modern yang umum dipakai di industri:
...
| [Bcrypt.js](https://www.npmjs.com/package/bcryptjs)                    | `^3.0.3`  | _Hashing_ password sebelum disimpan ke database. Password tidak boleh disimpan dalam bentuk teks asli (_plaintext_).                  |
...
| [Express Rate Limit](https://www.npmjs.com/package/express-rate-limit) | `^8.7.0`  | Membatasi jumlah _request_ dari satu IP dalam rentang waktu tertentu untuk mencegah serangan _brute force_.                           |
...
Setiap _request_ dari client melewati lapisan-lapisan berikut secara berurutan:
...
> **Catatan:** File `.env` sudah terdaftar di `.gitignore` sehingga tidak akan ter-_commit_ ke repository. Jangan pernah menyimpan kredensial sensitif di dalam kode.

**4. Jalankan server**

Untuk mode development (dengan _hot reload_ via Nodemon):
...
Dengan cara ini, kamu akan memahami bagaimana sebuah request diolah dari ujung ke ujung (_end-to-end_).
